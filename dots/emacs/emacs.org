...
#+RESULTS:
| (lambda (session) (when (dap--session-running session) (+dap-running-session-mode 1))) | dap-ui--stack-frame-changed | (lambda (session) (if (dap--session-running session) (progn (+dap-running-session-mode 1)))) |
